...
while True:
    ...
    img = cv2.imread('assets/color_spectrum.png')
    ...
    # show the image
    cv2.imshow('color spectrum', img)
    # set the mouse callback
    cv2.setMouseCallback('color spectrum', mouse_callback)
    cv2.createTrackbar('Value', 'color spectrum', 0, 100, slide)
    k = cv2.waitKey(1)
    if k & 0xFF == 27:
        break
    if k == ord('p'):
